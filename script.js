...
function renderCatalog() {
    ...
    container.innerHTML = items.map(p => {
        ...
        <div class="card" onclick="openModal(${p.myId})">
            ...
            <div class="card-info">
                <small style="color:var(--mono-coyote)">${p.SubCategory || p.Category}</small>
                <h4>${p.Name}</h4>
                <div class="price-box">
...
function updateModalGallery() {
    ...
    document.getElementById('modal-thumbnails').innerHTML = currentModalPics.map((src, i) => 
        `<img src="${src}" class="${i === currentModalPicIndex ? 'active' : ''}" onclick="setModalPic(${i})">`).join('');
}
    ...
    if (cart.length === 0) {
        ...
        content.innerHTML = cart.map((it, i) => {
            ...
            return `<div style="padding:10px 0; border-bottom:1px solid #222;">
                ...
                <span onclick="removeFromCart(${i})" style="color:red; float:right; cursor:pointer">✕</span>
            </div>`;
        }).join('');
...
function toggleMobileMenu(s) { document.getElementById('mobile-menu').classList.toggle('active', s); document.getElementById('body-overlay').classList.toggle('active', s); }
function toggleCart(s) { document.getElementById('cart-sidebar').classList.toggle('active', s); document.getElementById('body-overlay').classList.toggle('active', s); }
